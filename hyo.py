...
openai.api_key = st.secrets.get("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") or os.getenv("OPENAI_API_KEY")
...
def get_ai_mentor_response(user_input):
    ...
    prompt = f"""
You are an intelligent career mentor AI. Give personalized career advice based on this user profile and question.

User Profile: {profile}
User Context: {context}
User Question: {user_input}

Answer in a clear, personalized, actionable way.
"""
...
def user_profile_form():
    st.header("Create Your Unique Career Profile")
    ...
    if st.button("Save Profile"):
        if not name or not career_goal:
            ...
            st.success("Profile saved! Use the sidebar menu to navigate.")

def skill_rating_form():
    st.subheader("Personalize Your Skills and Rate Proficiency")
...
def show_dashboard():
    ...
    st.header(f"Welcome, {prof['name']}!")
    st.write(f"Profession: {prof['profession']}")
    st.write(f"Experience: {prof['experience_years']} years")
    st.write(f"Career Goals: {prof['career_goals']}")

    if not st.session_state.personal_skills:
        st.info("Please personalize your skills in 'Skills & Courses' tab.")
        return
...
def show_skills_courses():
    ...
    platforms = sorted(set(course['platform'] for course in learning_resources))
...
def show_mentor_chat():
    st.header("AI-Powered Mentor Chatbot")
    ...
    if user_input:
        ...
        if "timeline:" in low_text:
            ...
            response = f"Timeline noted: {timeline}."
...
def show_industry_news():
    ...
    if industry:
        articles = fetch_industry_news(industry)
        if articles:
            for title, url in articles:
                st.markdown(f"- [{title}]({url})")
        else:
            st.info("No news available or error fetching data.")
